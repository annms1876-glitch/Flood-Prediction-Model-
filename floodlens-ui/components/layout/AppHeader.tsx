...
export function AppHeader() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 h-16 border-b border-[#e3dfd5] bg-[#f8f7f5]/95 backdrop-blur-xl">
    <div className="mx-auto flex h-full max-w-[1200px] items-center gap-3 px-4 sm:px-6 lg:px-8">
      ...
      <nav className="hidden items-center gap-1 lg:flex"><Link href="/" className={`rounded-lg px-3 py-2 text-sm transition ${pathname === "/" ? "bg-[#e3dfd5] font-semibold text-[#261b07]" : "text-[#61594a] hover:bg-white"}`}>{t("overview")}</Link><Link href="/3d-map-view" className={`rounded-lg px-3 py-2 text-sm transition ${pathname.includes("map") ? "bg-[#e3dfd5] font-semibold text-[#261b07]" : "text-[#61594a] hover:bg-white"}`}>{t("liveMap")}</Link><Link href="/safety-tips" className="rounded-lg px-3 py-2 text-sm text-[#61594a] transition hover:bg-white">{t("preparedness")}</Link>{user?.email === "somenbarik75@gmail.com" && <Link href="/admin-dashboard" className="rounded-lg px-3 py-2 text-sm text-[#8f897e] transition hover:bg-white">Command centre</Link>}</nav>
      <div className="ml-auto flex items-center gap-2"><div className="hidden items-center gap-0.5 rounded-lg border border-[#e3dfd5] bg-white p-0.5 sm:flex"><Languages className="ml-1.5 h-3.5 w-3.5 text-[#8f897e]" /><button onClick={() => setLanguage("en")} className={`rounded px-2 py-1 text-[11px] font-semibold ${language === "en" ? "bg-[#f8da9d] text-[#261b07]" : "text-[#8f897e]"}`}>EN</button><button onClick={() => setLanguage("hi")} className={`rounded px-2 py-1 text-[11px] font-semibold ${language === "hi" ? "bg-[#f8da9d] text-[#261b07]" : "text-[#8f897e]"}`}>हिन्दी</button></div><button onClick={toggleTheme} className="flex h-9 w-9 items-center justify-center rounded-lg border border-[#e3dfd5] bg-white text-[#61594a]" aria-label="Toggle theme">{theme === "dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</button><Link href={isAuthority ? "/alert-management" : "/dashboard"} className="relative flex h-9 w-9 items-center justify-center rounded-lg border border-[#e3dfd5] bg-white text-[#61594a]" aria-label="Alerts"><Bell className="h-4 w-4" /><span className="absolute -right-1 -top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#f0624f] px-1 text-[9px] font-semibold text-white">12</span></Link><Link href="/sos-emergency" className="hidden h-9 items-center gap-1.5 rounded-lg bg-[#261b07] px-3 text-xs font-semibold text-white transition hover:bg-[#61594a] sm:flex"><ShieldCheck className="h-3.5 w-3.5 text-[#f9a600]" /> SOS</Link>{user ? <button onClick={openProfileModal} className="hidden h-9 items-center gap-2 rounded-lg border border-[#e3dfd5] bg-white px-3 text-xs font-semibold text-[#61594a] md:flex">{profile?.name || user.email?.split("@")[0]}</button> : <button onClick={openAuthModal} className="flex h-9 items-center gap-1.5 rounded-lg border border-[#261b07] px-3 text-xs font-semibold text-[#261b07]"><LogIn className="h-3.5 w-3.5" /> Sign in</button>}</div>
    </div>
  </header>;
}

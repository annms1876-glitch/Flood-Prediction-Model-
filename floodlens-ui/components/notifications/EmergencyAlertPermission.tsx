...
import { listenForForegroundFloodAlerts, registerFloodPushToken, speakFloodAlert } from "@/lib/notifications";
...
export function EmergencyAlertPermission() {
  ...
  useEffect(() => { let unsubscribe: (() => void) | undefined; void listenForForegroundFloodAlerts((data) => { if (data.type === "voice_alert") speakFloodAlert(data.voiceText || data.body || "Critical flood alert. Check Umeed AI now."); }).then((fn) => { unsubscribe = fn; }); return () => unsubscribe?.(); }, []);
